...
// ── File type detection (mirrors backend) ────────────────────────────────────
...
export default function MemoryPage() {
  ...
  const TABS = [
    ...
    { id: "upload", label: "Upload File",    count: null },
    { id: "ingest", label: "Ingest Data",    count: null },
    { id: "log",    label: "Ingestion Log",  count: log.length },
  ] as const;
    ...
    <div className="flex h-screen">
      ...
      <main className="flex-1 ml-14 flex flex-col min-h-0">
        ...
        <header className="flex items-center justify-between px-6 py-4 border-b border-rim flex-shrink-0">
          <div>
            <h1 className="text-text-primary font-semibold text-sm">Memory</h1>
            <p className="text-text-muted text-xs">Review auto-extracted facts · Upload files · Ingest data</p>
          </div>
        ...
        <div className="flex border-b border-rim px-6 flex-shrink-0">
          {TABS.map(({ id, label, count }) => (
            ...
            >
              {label}
              {count !== null && count > 0 && (
                <span className="ml-1.5 px-1.5 py-0.5 rounded-full bg-ember/20 text-ember text-[10px] font-mono">{count}</span>
              )}
            </button>
          ))}
        </div>
            ...
            <>
              {loading ? (
                ...
                <div className="flex flex-col items-center justify-center h-48 gap-2">
                  <CheckCircle size={24} className="text-green-400/50" />
            ...
            <div className="max-w-lg space-y-5">
              ...
              <div className="flex flex-wrap gap-2">
                {Object.entries(FILE_TYPE_MAP)
                  ...
                  .map(([ext, meta]) => (
                    <span key={ext} className={`text-[10px] font-mono px-2 py-1 rounded-lg border ${meta.color} ${meta.bg} ${meta.border}`}>
                      {meta.label}
                    </span>
                  ))}
              </div>
                ...
                {file && detectedType ? (
                  <div className="flex flex-col items-center gap-3">
                    ...
                    <div className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg border text-xs font-mono ${detectedType.color} ${detectedType.bg} ${detectedType.border}`}>
                      ...
                      <span className="text-[10px] opacity-70">{detectedType.desc}</span>
                    </div>
                    <button
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full bg-elevated border border-rim rounded-lg px-3 py-2 text-text-primary text-sm placeholder:text-text-muted font-mono outline-none focus:border-dawn/50 transition-colors"
                />
              </div>
              ...
              <button
                ...
                className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-dawn/90 hover:bg-dawn text-abyss text-sm font-medium disabled:opacity-40 transition-all"
              >
                <Upload size={14} />
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    <input value={repoPath} onChange={(e) => setRepoPath(e.target.value)}
                      ...
                      className="w-full bg-elevated border border-rim rounded-lg px-3 py-2 text-text-primary text-sm placeholder:text-text-muted font-mono outline-none focus:border-dawn/50" />
                  </div>
                  <div>
                    <label className="text-text-secondary text-xs block mb-1.5">Repo name</label>
                    <input value={repoName} onChange={(e) => setRepoName(e.target.value)}
                      placeholder="Sentinel Trading Bot"
                      className="w-full bg-elevated border border-rim rounded-lg px-3 py-2 text-text-primary text-sm placeholder:text-text-muted outline-none focus:border-dawn/50" />
                  </div>
                </div>
              ) : (
                <div className="space-y-3">
                  ...
                  <div>
                    ...
                    <textarea value={docContent} onChange={(e) => setDocContent(e.target.value)}
                      ...
                      className="w-full bg-elevated border border-rim rounded-lg px-3 py-2 text-text-primary text-sm placeholder:text-text-muted outline-none focus:border-dawn/50 resize-none" />
                  </div>
                </div>
              )}
                ...
                log.map((entry) => (
                  <div key={entry.id} className={`bg-surface border rounded-xl px-4 py-3 ${entry.status === "success" ? "border-rim" : "border-ember/30"}`}>
                    <div className="flex items-center justify-between gap-2">
                      ...
                      <span className="text-text-muted text-[10px] font-mono">{new Date(entry.ingested_at).toLocaleString()}</span>
                    </div>
                    <p className="text-text-muted text-xs font-mono mt-1 truncate">{entry.source_ref}</p>
                    {entry.status === "success" && (
                      <p className="text-text-secondary text-[11px] mt-1">{entry.nodes_created} nodes · {entry.edges_created || 0} edges</p>
                    )}
                    {entry.error && <p className="text-ember text-[11px] mt-1 font-mono">{entry.error}</p>}
                  </div>
                ))
              )}
            </div>
